...
int CalculateNum(PathNode *root);
...
PQNode *SortPaths(PQNode PQptr[])
{    
    int i, j,a=0,b=0,sz=238,c=0,d=0;
    PathNode *temp=NULL;
   ...
   for (i = 0; i < sz-1; i++)       
  {
       ...
       for (j = 0; j < sz-i-1; j++)  
           {  a = CalculateCost(PQptr[j].root);
              ...
              c=CalculateNum(PQptr[j].root);
              d=CalculateNum(PQptr[j+1].root);
...
{  
    ...
    filePointer = fopen("mazeoutput.txt", "w");
