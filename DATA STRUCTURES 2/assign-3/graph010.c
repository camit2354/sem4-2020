...
status AddEdgeToMst(graph *gproot, char startVertexName, char endVertexName ,int weight);
    ...
    while(count<N-1)
    {
        simplyDisplay(gp1);
           ...
           EdgeNode *eg = LowestWeightEdge(gp1);
            ...
            {
                AddEdgeToMst(gproot,startVertexName,endVertexName,eg->weight);
...
status AddEdgeToMst(graph *gproot, char startVertexName, char endVertexName ,int weight)
{
    ...
    while(gp1 != NULL && isVertexPresent(gp1,startVertexName) == FALSE)
    {
        gp1 = gp1->next;
    }

     while(gp2 != NULL && isVertexPresent(gp2,endVertexName) == FALSE)
    {
        gp2 = gp2->next;
    }
    ...
    {  printf("hi1");
        ...
        {
           //union of two graphs 
           VertexNode *iv = gp1->VertexListRoot;
           while(iv->down != NULL)
           {
               iv = iv->down;
           }
           iv->down = gp2->VertexListRoot;
    ...
    {           graph *gp = (graph *)malloc(sizeof(graph));
                addVertex(gp,startVertexName);
                addVertex(gp,endVertexName);
                addEdge(gp,startVertexName,endVertexName,weight);
                addEdge(gp,endVertexName,startVertexName,weight);
...
{
    VertexNode *vptr = gp->VertexListRoot;
    while(vptr != NULL && vptr->vertexName != vertexName)
    {
        vptr = vptr->down;
    }
     ...
     VertexNode *vptr = gp->VertexListRoot;
        ...
        if(vptr->mark == 0)
        {
            ...
            else
            {
                ...
                {
                    retval = DepthFirstAlgo(gp,cnPtr->vertexName,endPoint);
                    cnPtr = cnPtr->right;
                }
                /* code */
            }
...
graph *addEdge(graph *graphPtr, char startNodeName ,char endNodeName ,int weight)
{
    ...
    while(ptr2 != NULL && ptr2->vertexName != startNodeName)
    {
        ptr2 = ptr2->down;
    }
...
boolean isPathExists(graph *graphPtr ,char startNodeName, char endNodeName)
{
    ...
    else
    {
        ...
        {
            retval = isPathExists(graphPtr,cnPtr->vertexName,endNodeName);
            cnPtr = cnPtr->right;
        }
...
void displayDepthFirst(graph *graphPtr , char startPoint)
{
    ...
    else
    {
        ...
        while(cnPtr != NULL)
        {
            displayDepthFirst(graphPtr,cnPtr->vertexName);
            cnPtr = cnPtr->right;
        }
