...
int get_dedicated_wing_for_new_parked_vehicle(char vehicle_number[9],mapped_tree_node_type *mapped_tree_ptr,parking_lot_status_type **parking_lot_status_pptr);
int get_dedicated_index_for_new_parked_vehicle(char vehicle_number[9],mapped_tree_node_type *mapped_tree_ptr,parking_lot_status_type **parking_lot_status_pptr);
    ...
    while(fscanf(fp,"%d %s %s %s",vehicle_type,intime,indate,vehicle_number)!=EOF)
    {
        ...
        if(dayNode == NULL)
        {
            ...
            strcpy(newParkedVehicleNode->vehicle_number,vehicle_number);
                  dedicated_wing = get_dedicated_wing_for_new_parked_vehicle();
                  dedicated_index = get_dedicated_index_for_new_parked_vehicle();
               if(vehicle_type ==2 )
               {
