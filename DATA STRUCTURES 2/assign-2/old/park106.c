...
void initialize_parking_lot(parking_lot_status_type **parking_lot_status_pptr);
...
void main()
{
    ...
    initialize_parking_lot(&parking_lot_status_ptr);
   ...
   initialize_flat_info_tree(&map_space_ptr,&mapped_tree_ptr,&flat_info_tree_ptr);
  // preOrderMappedTree(mapped_tree_ptr);
   // preOrderFlatInfoTree(flat_info_tree_ptr);

  initialize_parking_db_day_tree(mapped_tree_ptr,&parking_lot_status_ptr,&parking_db_day_tree_ptr);
  preOrderParkingDbDayTree(parking_db_day_tree_ptr);
//    show_current_map_space_index(map_space_ptr);
   Parking_status(parking_lot_status_ptr);
...
/*   ................... preorder function..................*/
void preOrderParkingDbDayTree(parking_db_day_type *root)
{ // printf("\nfun :preorder parking db day tree \n");
    if(root != NULL) 
    { 
        printf("\n date = %s  top = %d  \n",root->indate,root->top); 
...
{ 
    if(root != NULL) 
    { 
        printf("\n vehicle number =%s\t,wing_d = %d ,flat_id =%d ,dedicated wing = %d ,vehicle compact index = %d  ,large index =%d  ,key =%d ,\n", root->vehicle_number,root->wing_id,root->flat_id,root->dedicated_parking_wing,root->compact_spot_index,root->large_spot_index,root->key); 
...
{   
   ...
   for(int i =0;i<9;i++)
    {
     ...
     // printf("\n%s %s\n..................\n",newNode->vehicle_number[0],newNode->vehicle_number[1]);
  ...
  {   if(j==0)
      {
          dedicated_parking_wing = newNode->wing_id ;
           mapped_tree_node_type *newMappedTreeNode = new_mapped_tree_node(newNode->wing_id,dedicated_parking_wing,newNode->flat_id,4,(*map_space_pptr)->large_space[dedicated_parking_wing-1],newNode->vehicle_number[i]);
      ...
      {
           dedicated_parking_wing = (newNode->wing_id )%3 +1 ;
            mapped_tree_node_type *newMappedTreeNode = new_mapped_tree_node(newNode->wing_id,dedicated_parking_wing,newNode->flat_id,2,(*map_space_pptr)->large_space[dedicated_parking_wing-1],newNode->vehicle_number[i]);
             (*map_space_pptr)->large_space[dedicated_parking_wing-1] = (*map_space_pptr)->large_space[dedicated_parking_wing-1] +1;
              *mapped_tree_pptr = insert_mapped_tree(*mapped_tree_pptr , newMappedTreeNode);  
...
{ // printf("\n i am in function preOrderFlatInfoTree\n");
    if(root != NULL) 
    { 
        printf(" \n wing id=%d, flat id =%d key =%d \n", root->wing_id,root->flat_id,root->key); 
...
void  Parking_status (parking_lot_status_type *parking_lot_ptr)
{
     ...
     printf("\n wing A occupied compact spots are below\n");
     for(int i=0;i<36;i++)
     {
     ...
     printf("\n wing A occupied large spots are below\n");
     for(int i=0;i<36;i++)
     {
      ...
      printf("\n wing B free occupied spots are below\n");
      for(int i=0;i<36;i++)
     {
     ...
     printf("\n wing B occupied large spots are below\n");
     for(int i=0;i<36;i++)
     {
      ...
      printf("\n wing C  occupied compact spots are below\n");
      for(int i=0;i<36;i++)
     {
     ...
     printf("\n wing C occupied large spots are below\n");
     for(int i=0;i<36;i++)
     {
...
void initialize_parking_lot(parking_lot_status_type **parking_lot_status_pptr)
{
    *parking_lot_status_pptr = (parking_lot_status_type *)malloc(sizeof(parking_lot_status_type));
    (*parking_lot_status_pptr)->wing_a_ptr = (parking_wing_status_type *)malloc(sizeof(parking_wing_status_type));
    (*parking_lot_status_pptr)->wing_b_ptr = (parking_wing_status_type *)malloc(sizeof(parking_wing_status_type));
    (*parking_lot_status_pptr)->wing_c_ptr = (parking_wing_status_type *)malloc(sizeof(parking_wing_status_type));

     ((*parking_lot_status_pptr)->wing_a_ptr)->parking_wing =1;
     ((*parking_lot_status_pptr)->wing_b_ptr)->parking_wing =2;
     ((*parking_lot_status_pptr)->wing_c_ptr)->parking_wing =3;
