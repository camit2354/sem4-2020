...
typedef struct flat_info_tag
{
    ...
    char *vehicle_number[max_vehicle];
...
{ // printf("\n i am in function initialize_flat_info_tree \n");
  ...
  while(fscanf(fp, "%d\t%d\t%d\t%d\t%s\t%s",&wing_id,&flat_id,&no_of_two_wheelers,&no_of_four_wheelers,vehicle_number0,vehicle_number1)!=EOF  )
  {   
    
    flat_info_type *newNode =(flat_info_type *)malloc(sizeof(flat_info_type));
    printf("\n @ : %d %d %d %d %s %s\n",wing_id,flat_id,no_of_two_wheelers,no_of_four_wheelers,vehicle_number0,vehicle_number1);
    ...
    newNode->total_no_of_vehicles =0;
      ...
      newNode->vehicle_number[0]  = vehicle_number0;
      newNode->vehicle_number[0]  = vehicle_number1;
   ...
   if(vehicle_type ==2 )
   {
           flat_node->no_of_two_wheelers = flat_node->no_of_two_wheelers +1;
            flat_node->total_no_of_vehicles = flat_node->total_no_of_vehicles  +1; 
             ...
             for(int i=0; i<9; i++)
             {
              flat_node->vehicle_number[flat_node->total_no_of_vehicles -1][i]  = vehicle_number[i] ;
             }

             printf("\n %s \n",flat_node->vehicle_number[flat_node->total_no_of_vehicles -1]);
